...
セッションは互いを見られない。同じPRを2つのセッションが並行実装しても、
気づけるのは GitHub 上だけである。**着手した印を、実装より先に GitHub へ置く。**

1. 同じP番号のブランチが既にあるか確認する。

   ```bash
   git ls-remote --heads origin 'claude/p<N>-*'
   ```

2. 同じP番号の PR（open / merged の両方）が既にあるか確認する。
3. **どちらかが見つかったら実装しない。** 何が既にあるかを報告し、指示を待つ。
4. 何も無ければ、指示書の「担当ブランチ」欄に自分のブランチ名を書いてコミットし、
   **実装を始める前に Draft PR を作る。** タイトルの先頭にP番号を入れる（例: `P4: ...`）。
   これが他セッションから見える唯一の『着手済み』の印である。

## PR運用

- **1 PR ＝ 1縦切り。** 前のPRの受入基準が満たされるまで次に進まない。
- **着手時に Draft PR、完成時に Ready for review。** ブランチへの push だけで終わらせない。

  ```bash
  gh pr create --base main --draft --fill   # 着手時
  gh pr ready                               # 完成時
  ```

  `gh` が使えない実行環境では、GitHub MCP ツールで同じことを行う。
